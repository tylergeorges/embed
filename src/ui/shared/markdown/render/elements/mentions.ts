...
const base = (inline: boolean, color: string, clickable: boolean) => css`
  ...
  ${inline
    ...
    : css`
        padding: 0 2px;
        background-color: ${Color(color).fade(color === Constants.THEME_COLOR_ACCENT ? .7 : .9).string()};
        color: ${color === Constants.THEME_COLOR_ACCENT ? Color('white').fade(.2).string() : color} !important;
