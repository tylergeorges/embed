...
import { List, Thread, NoThreads, Preview, ThreadName, NoThreadsIcon, Time, Root, Username, Separator } from './elements'
import stars from '@images/discordAssets/stars.svg'
import Tooltip from 'rc-tooltip'
import THREADS from '../../views/Messages/Header/ThreadBrowser/Threads.graphql'
            ...
            <List className="threads-list">
                {!loading
                    ? data?.channel?.threads?.length
                        ? <>{(data.channel.threads as Threads_channel_threads_ThreadChannel[])
                            ...
                            .map(thread => {
                                ...
                                const firstMessage = thread.messageBunch.messages[0]
                                ...
                                return <Thread key={thread.id} onClick={() => {
                                    ...
                                    <Preview>
                                        <ReplyUserBase>
                                            <Username color={color}>{firstMessage.author.name}</Username>
                                        </ReplyUserBase>
                                        <Separator>:</Separator>
                                        <Content message={firstMessage} isReplyContent={true} />
                                    </Preview>
                                    <Time isReplyContent={true}> {short(moment(lastMessage.createdAt))}</Time>
                                </Thread>
                            }
                        )}</>
